...
function saveScore(slug: string, score: number) {
...
function makeArt(slug: string, ac: string): string {
  if (slug === 'vector-duel') {
    return `<svg viewBox="0 0 200 140" xmlns="http://www.w3.org/2000/svg"
      style="width:100%;height:100%;opacity:.7;overflow:visible">
      <defs>
        <style>
          .vd-ship { transform-origin:100px 70px; animation: ship-spin 12s linear infinite; }
          .vd-r1   { transform-origin:100px 70px; animation: rock-orbit-cw 8s linear infinite; }
          .vd-r2   { transform-origin:100px 70px; animation: rock-orbit-ccw 13s linear infinite; }
          .vd-r3   { transform-origin:100px 70px; animation: rock-orbit-cw 18s linear infinite reverse; }
          @keyframes ship-spin {
            from { transform: rotate(0deg) } to { transform: rotate(360deg) }
          }
          @keyframes rock-orbit-cw {
            from { transform: rotate(0deg) } to { transform: rotate(360deg) }
          }
          @keyframes rock-orbit-ccw {
            from { transform: rotate(0deg) } to { transform: rotate(-360deg) }
          }
        </style>
      </defs>
  ...
  // generic starfield for unknown slugs
...
function buildStarfield(container: HTMLElement) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    s.style.cssText = `left:${x}%;width:${size}px;height:${size}px;
...
// add "coming soon" ghost cards to fill the row
...
for (let i = 0; i < soonsNeeded; i++) {
  ...
  ghost.innerHTML = `
    <div class="card-screen">
      <span class="card-screen-title" style="color:#222233">???</span>
    </div>
    <div class="card-body">
...
async function launchGame(slug: string) {
  ...
  // reset controls hint (re-trigger animation by replacing element)
